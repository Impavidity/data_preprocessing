...
def get_query_columns(query: str) -> List[str]:
    ...
    # and not "reset" previously found SELECT keyword
    functions_ignored = ['COUNT', 'MIN', 'MAX', 'FROM_UNIXTIME', 'DATE_FORMAT', 'CAST', 'CONVERT', 'SUM', 'AVG']
...
def _update_table_names(tables: List[str],
    ...
    if last_keyword in ['FROM', 'JOIN', 'INNER JOIN', 'FULL JOIN', 'FULL OUTER JOIN', 'NATURAL JOIN',
...
def get_query_tables(query: str) -> List[str]:
    ...
    table_syntax_keywords = [
        # SELECT queries
        'FROM', 'WHERE', 'JOIN', 'INNER JOIN', 'FULL JOIN', 'FULL OUTER JOIN', "NATURAL JOIN",
        'LEFT OUTER JOIN', 'RIGHT OUTER JOIN',
...
def normalize_likes(sql: str) -> str:
    ...
    # LIKE '%bot'
    sql = re.sub(r"LIKE '[^\']+'", 'LIKE _VALUE_', sql)

    # or all_groups LIKE X or all_groups LIKE X
    matches = re.finditer(r'(or|and) [^\s]+ LIKE _VALUE_', sql, flags=re.IGNORECASE)
...
def generalize_sql(sql: Optional[str]) -> Optional[str]:
    ...
    # All numbers => N
    sql = re.sub(r' [+-]?(\d*\.)?\d+', ' _VALUE_', sql)

    # WHERE foo IN ('880987','882618','708228','522330')
    sql = re.sub(r' (IN|VALUES)\s*\([^,]+,[^)]+\)', ' \\1 (_VALUES_)', sql, flags=re.IGNORECASE)
